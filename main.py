...
df = pd.read_csv("valid_urls_100.csv")
...
model.load_state_dict(torch.load('img_classifier3.pth'))

model.eval()
print('resnet152 was loaded!!!!!')
...
def simple_demo():
	...
	img_label = outputs.softmax(dim=1).argmax(dim=1).item()
	print("label is ", img_label)
	#print("head is :", df.head())
	predicted_img_url = df.loc[df["id"] == img_label, 'url'].item()
	print("url is ", predicted_img_url)
	response = http_requests.get(predicted_img_url)
	file = io.BytesIO(response.content)
	outimg = Image.open(file).convert('RGB')
	# print(outputs.shape)
	...
	output_image_str = image2string(outimg)
